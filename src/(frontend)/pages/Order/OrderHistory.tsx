...
const ViewOrders: React.FC<OrderHistoryProps> = ({ currentUser }) => {
    ...
    useEffect(() => {
        const fetchOrders = async () => {
            try {
                ...
                const filtered = allOrders.filter((o: WholesaleOrder) => {
                    const matchesCompany = companyId ? String(o.company_id) === companyId : true;
                    const isPaidAndFulfilled = o.payment_status === "Paid" && o.fulfillment_status === "Fulfilled"; // ✅
                    return matchesCompany && isPaidAndFulfilled;
                });
    ...
    // Add this helper to get available years from orders
    const availableYears = [...new Set(orders.map(o => new Date(o.wholesale_orders_date).getFullYear()))].sort((a, b) => b - a);

    // Update your filtered variable to include the date filter
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-zinc-400 mt-1">View all completed wholesale orders (Paid & Fulfilled)</p>
                </div>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3">
                ...
                <div className="flex items-center bg-zinc-900 border border-zinc-800 rounded-lg p-1 gap-1">
                    ...
                    <button
                        ...
                        className={`px-3 py-1.5 rounded-md text-sm font-medium transition-colors ${filterType === "year"
                            ? "bg-indigo-600 text-white"
                            : "text-zinc-400 hover:text-white"
                            }`}
                    >
                    ...
                    <button
                        ...
                        className={`px-3 py-1.5 rounded-md text-sm font-medium transition-colors ${filterType === "month"
                            ? "bg-indigo-600 text-white"
                            : "text-zinc-400 hover:text-white"
                            }`}
                    >
                ...
                {/* Year Selector — shown for both "year" and "month" filters */}
                ...
                <span className="text-zinc-500 text-sm">
                    {filterType === "all" && "Showing all completed orders"}
