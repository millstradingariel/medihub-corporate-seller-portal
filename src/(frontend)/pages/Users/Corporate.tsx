...
import { UserCog, Plus, X, Mail, Shield, Calendar, Search, Loader2, Edit, Archive, AlertTriangle, CheckCircle, XCircle, Clock } from 'lucide-react';
...
const AdminUsers: React.FC<AdminUsersProps> = ({ currentUser }) => {
    ...
    // Fetch corporate roles on mount
    useEffect(() => {
        fetchCorporateRoles();
    }, []);

    // Fetch corporate roles (role_type = 'corporate')
    const fetchCorporateRoles = async () => {
        try {
            const token = localStorage.getItem('supabaseToken');
            const res = await fetch(`${API_URL}/api/roles?role_type=corporate`, {
    ...
    const fetchUsers = async () => {
        try {
            setLoading(true);
            const token = localStorage.getItem('supabaseToken');
            const res = await fetch(`${API_URL}/api/user/corporate/view`, {
                headers: { 'Authorization': `Bearer ${token}` }
            });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const url = isEditing
                ? `${API_URL}/api/user/corporate/${editingUser.id}/update`
                : `${API_URL}/api/user/corporate/create`;
            const method = isEditing ? 'PUT' : 'POST';

            const body = isEditing
                ? { name: formData.name, role_id: parseInt(formData.role_id), is_active: formData.is_active } // ✅
                : { name: formData.name, email: formData.email, password: formData.password, role_id: parseInt(formData.role_id), is_active: formData.is_active };

            if (!isEditing) {
                if (!formData.name) { setFormError('Name is required'); setSubmitting(false); return; }
                if (!formData.email) { setFormError('Email is required'); setSubmitting(false); return; }
                if (!formData.password) { setFormError('Password is required'); setSubmitting(false); return; }
                if (!formData.role_id || isNaN(parseInt(formData.role_id))) {
    ...
    const handleArchiveClick = (user: AdminUser) => {
        setConfirmModal({
            isOpen: true,
            userId: user.id,
            userName: user.name,
                ...
                <div className="grid grid-cols-1 sm:grid-cols-4 gap-3 sm:gap-4">
                    ...
                    <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4 sm:p-6">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <p className="text-zinc-400 text-xs sm:text-sm">Total Admins</p>
                    ...
                    <div className="flex flex-col gap-3 sm:gap-4">
                        ...
                        <div className="relative w-full">
                            ...
                            <input
                                ...
                                className="w-full pl-10 pr-4 py-2 text-sm bg-zinc-950 border border-zinc-800 rounded-lg text-white placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-purple-500"
                            />
                        </div>
                    </div>
                </div>
                        ...
                        <table className="w-full min-w-[750px]">
                            ...
                            <tbody className="divide-y divide-zinc-800">
                                {filteredUsers.length === 0 ? (
                                    <tr>
                                        <td colSpan={6} className="p-6 sm:p-8 text-center text-sm text-zinc-500">
                                            No admins found
                                        </td>
                                    </tr>
                                ) : (
                            ...
                            <div key={user.id} className="bg-zinc-900 border border-zinc-800 rounded-lg p-4 space-y-3">
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    <div>
                                        ...
                                        <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium text-white capitalize">
                                            {user.role_display_name || user.role_name}
                                        </span>
                                    </div>
                                    <div>
                                        ...
                                        <div className="flex items-center gap-1">
                                            {user.is_active === 1 ? (
                                                <>
                                                    ...
                                                    <p className="text-sm text-indigo-400 font-medium">Active</p>
                                                </>
                                            ) : user.is_active === 0 ? (
                                                <>
                                                    <div className="w-2 h-2 bg-indigo-500 rounded-full"></div>
                                                    <p className="text-sm text-indigo-400 font-medium">Archived</p>
                                                </>
                                            ) : (
                                                <>
                                                    <div className="w-2 h-2 bg-indigo-500 rounded-full"></div>
                                                    <p className="text-sm text-indigo-400 font-medium">Pending</p>
                                                </>
                                            )}
                                        </div>
                                    </div>
                            ...
                            <form onSubmit={handleSubmit} className="p-4 sm:p-6 space-y-3 sm:space-y-4">
                                <div>
                                    ...
                                    <input
                                        ...
                                        className="w-full px-3 sm:px-4 py-2 text-sm bg-zinc-950 border border-zinc-800 rounded-lg text-white placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                                        required
                                    />
                                </div>
                                ...
                                <div>
                                    ...
                                    <select
                                        ...
                                        className="w-full px-3 sm:px-4 py-2 text-sm bg-zinc-950 border border-zinc-800 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-indigo-500"
                                        required
                                    >
                                        {roles.length === 0 ? (
                                    ...
                                    <p className="text-xs text-zinc-500 mt-1">
                                        Only corporate roles are shown
                                    </p>
                                </div>
